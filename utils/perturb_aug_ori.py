...
parser.add_argument('--task', type=str, default="xnli", choices=["pawsx", "xnli"])
parser.add_argument('--input_dir', type=str, default="/Data/data_generalized")
parser.add_argument('--output_dir', type=str, default="/Data/AugWithOrign/xnli")
parser.add_argument('--neighbors', type=str, default="/Data/counterfitted_neighbors.json")
...
with open(os.path.join(args.input_dir, args.task, f"train-en.tsv"), encoding='UTF-8') as fp:
    lines = fp.readlines()

n_perturb = 0
n_total = len(lines)

with open(os.path.join(args.output_dir, args.task, f"train-en.tsv"), "w", encoding='UTF-8') as fp:
    for line in tqdm(lines, ascii=True):
        ...
        for p_sent1, p_sent2 in zip(p_sents1, p_sents2):
            # print(f"{p_sent1}\t{p_sent2}\t{label.strip()}\t{s1}\t{s2}\t{label}")
            fp.write(f"{p_sent1}\t{p_sent2}\t{label.strip()}\t{s1}\t{s2}\t{label}")
...
shutil.copyfile(os.path.join(args.input_dir, args.task, "dev-en.tsv"),
                os.path.join(args.output_dir, args.task, "dev-en.tsv"))
for lang in langs:
    shutil.copyfile(os.path.join(args.input_dir, args.task, f"test-{lang}.tsv"),
                    os.path.join(args.output_dir, args.task, f"test-{lang}.tsv"))
